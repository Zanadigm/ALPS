				...
				<tbody>
					...
					$qry = $conn->query("SELECT po.*, s.name as sname FROM `po_list` po inner join `supplier_list` s on po.supplier_id = s.id order by unix_timestamp(po.date_updated)");
						while($row = $qry->fetch_assoc()):
							...
							$row['total_amount'] = $conn->query("SELECT sum(o.quantity * i.unit_price) as total FROM order_items o inner join item_list i on o.item_id = i.id where po_id = '{$row['id']}'")->fetch_array()['total'];
					?>
						<tr>
