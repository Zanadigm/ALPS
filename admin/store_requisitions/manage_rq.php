				...
				<div class="col-md-12">
					<table class="table table-striped table-bordered" id="item-list">
						...
						<tbody>
							<?php 
							if(isset($id)):
							$requested_items_qry = $conn->query("SELECT r.*,i.name,i.unit,i.selling_price, i.description FROM `requisition_items` r inner join item_list i on r.item_id = i.id where rq_id = '$id' ");
							echo $conn->error;
					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							<select name="status" id="status" class="form-control form-control-sm rounded-0">
								...
								<option value="2" <?php echo isset($status) && $status == 2 ? 'selected': '' ?>>Processing</option>
								<option value="3" <?php echo isset($status) && $status == 3 ? 'selected': '' ?>>Fulfilled</option>
							</select>
						</div>
					</div>
				</div>
